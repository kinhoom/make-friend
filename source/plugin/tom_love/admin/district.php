...
$values = array(intval($_GET['pid']), intval($_GET['cid']), intval($_GET['did']), intval($_GET['tid']));
$elems = array($_GET['province'], $_GET['city'], $_GET['district'], $_GET['towns']);
$level = 1;
$upids = array(0);
$theid = 0;
for($i=0;$i<4;$i++) {
...
if(submitcheck('editsubmit')) {
	...
	showformheader($modFromUrl.'&pid='.$values[0].'&cid='.$values[1].'&did='.$values[2].'&tid='.$values[3]);
	showtableheader();
	...
	$names = array('province', 'city', 'district','towns');
	for($i=0; $i<4;$i++) {
	...
	for($i=0;$i<4;$i++) {
		$l = $i+1;
		$jscall = ($i == 0 ? 'this.form.city.value=\'\';this.form.district.value=\'\';' : '')."refreshdistrict('$elems[0]', '$elems[1]', '$elems[2]', '$elems[3]')";
...
function refreshdistrict(province, city, district, towns) {
	location.href = "$adminurl"
		+ "&province="+province+"&city="+city+"&district="+district+"&towns="+towns
		+"&pid="+$(province).value + "&cid="+$(city).value+"&did="+$(district).value+"&tid="+$(towns).value;
}
