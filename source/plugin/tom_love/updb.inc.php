...
if(isset($_G['uid']) && $_G['uid'] > 0 && $_G['groupid'] == 1){
    ...
    if (!isset($tom_love_field['anlians'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `anlians` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['vip_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `vip_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['vip_time'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `vip_time` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['recommend_time'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `recommend_time` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['recommend_do_time'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `recommend_do_time` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['mate_demands'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `mate_demands` varchar(255) DEFAULT NULL;\n";
    }
    if (!isset($tom_love_field['last_smstp_time'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `last_smstp_time` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['hjcountry_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `hjcountry_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['hjprovince_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `hjprovince_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['hjcity_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `hjcity_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['hjarea_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `hjarea_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['towns_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `towns_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['hjtowns_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `hjtowns_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['hjarea'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `hjarea` varchar(255) DEFAULT NULL;\n";
    }
    if (!isset($tom_love_field['sign_time'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `sign_time` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['clicks'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `clicks` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['subscribe'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `subscribe` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['phone_renzheng'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `phone_renzheng` tinyint(4) NOT NULL DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['shenhe_status'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `shenhe_status` tinyint(4) NOT NULL DEFAULT '2';\n";
    }
    if (!isset($tom_love_field['member_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `member_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['unionid'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `unionid` varchar(255) DEFAULT NULL;\n";
    }
    if (!isset($tom_love_field['is_majia'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `is_majia` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_field['smstp_open'])) {
        $sql .= "ALTER TABLE `pre_tom_love` ADD `smstp_open` int(11) DEFAULT '1';\n";
    }
    
    $tom_love_shuoshuo_field = C::t('#tom_love#tom_love_shuoshuo')->fetch_all_field();
    if (!isset($tom_love_shuoshuo_field['reply_count'])) {
        $sql .= "ALTER TABLE `pre_tom_love_shuoshuo` ADD `reply_count` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_shuoshuo_field['zan_count'])) {
        $sql .= "ALTER TABLE `pre_tom_love_shuoshuo` ADD `zan_count` int(11) DEFAULT '0';\n";
    }
    
    $tom_love_guanxi_field = C::t('#tom_love#tom_love_guanxi')->fetch_all_field();
    if (!isset($tom_love_guanxi_field['time_key'])) {
        $sql .= "ALTER TABLE `pre_tom_love_guanxi` ADD `time_key` int(11) DEFAULT '0';\n";
    }
    
    $tom_love_renzheng_field = C::t('#tom_love#tom_love_renzheng')->fetch_all_field();
    if (!isset($tom_love_renzheng_field['pic_s'])) {
        $sql .= "ALTER TABLE `pre_tom_love_renzheng` ADD `pic_s` varchar(255) DEFAULT NULL;\n";
    }
...
EOF;
    ...
    if (!isset($tom_love_order_field['order_type'])) {
        $sql .= "ALTER TABLE `pre_tom_love_order` ADD `order_type` int(11) DEFAULT '1';\n";
    }
    if (!isset($tom_love_order_field['time_value'])) {
        $sql .= "ALTER TABLE `pre_tom_love_order` ADD `time_value` int(11) DEFAULT '0';\n";
    }
    $tom_love_flowers_log_field = C::t('#tom_love#tom_love_flowers_log')->fetch_all_field();
    if (!isset($tom_love_flowers_log_field['gift_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love_flowers_log` ADD `gift_id` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_flowers_log_field['toUser_id'])) {
        $sql .= "ALTER TABLE `pre_tom_love_flowers_log` ADD `toUser_id` int(11) DEFAULT '0';\n";
    }
    
    $tom_love_phb_field = C::t('#tom_love#tom_love_phb')->fetch_all_field();
    if (!isset($tom_love_phb_field['phb_type'])) {
        $sql .= "ALTER TABLE `pre_tom_love_phb` ADD `phb_type` int(11) DEFAULT '1';\n";
    }

    
    $tom_love_report_field = C::t('#tom_love#tom_love_report')->fetch_all_field();
    if (!isset($tom_love_report_field['status'])) {
        $sql .= "ALTER TABLE `pre_tom_love_report` ADD `status` int(11) DEFAULT '0';\n";
    }
    if (!isset($tom_love_report_field['report_pic_1'])) {
        $sql .= "ALTER TABLE `pre_tom_love_report` ADD `report_pic_1` varchar(255) DEFAULT NULL;\n";
    }
    if (!isset($tom_love_report_field['report_pic_2'])) {
        $sql .= "ALTER TABLE `pre_tom_love_report` ADD `report_pic_2` varchar(255) DEFAULT NULL;\n";
    }
    
    $tom_love_pic_field = C::t('#tom_love#tom_love_pic')->fetch_all_field();
    if (!isset($tom_love_pic_field['shenhe_status'])) {
        $sql .= "ALTER TABLE `pre_tom_love_pic` ADD `shenhe_status` tinyint(4) NOT NULL DEFAULT '1';\n";
    }
